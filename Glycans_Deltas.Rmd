...
DFs_for_GLMs <- DFs_Delta_Combined %>% 
...
for (i in unique(Multi_Results$Origin)) {
  ...
  Current_Plot_Estimates <- plot_summs(Model_1, 
                                       ...
                                       model.names = c("Clinical Response",
                                                       "Renal Impairment"), 
                                       coefs = setdiff(names(coef(Model_1)), "(Intercept)"),
...
ggplot(Multi_Results_Clean, aes(x = estimate, y = Marker, color = Model)) +
  ...
  labs(
    x     = "Log Odds",
    y     = "Predictor",
    # title = paste0("OR Comparison: ", name1, " vs. ", name2),
    color = "Model"
  ) +
  theme_minimal()
