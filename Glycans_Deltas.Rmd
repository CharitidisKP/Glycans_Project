...
Estimate_Plots <- Multi_Results_Firth_Clean %>%
  ...
  imap(function(df, name) {
    p <- ggplot(df, aes(x = estimate, y = Marker, color = Model)) +
...
Final_Estimate_Plot <- wrap_plots(list(Estimate_Plots_wrap, Plot_x_axis),
                         ncol = 1,
                         heights = c(sum(heights), 1), 
                         guides  = "collect") +
  plot_annotation(title = "Firth-Penalized logistic regression results - Estimates",
...
ggsave(plot = Final_Estimate_Plot,
       filename = "~/SLE/Glycans/Plots/Firth_GLM_Delta/Firth_Estimates_Total.jpeg",
       height = 12, width = 10, dpi = 300, device = "jpeg")
...
Estimate_Plots <- Multi_Results_Firth_Clean %>%
  ...
  set_names(map_chr(., ~ unique(.x$Origin))) %>%
  imap(function(df, name) {
    p <- ggplot(df, aes(x = OR, y = Marker, color = Model)) +
      geom_point(position = position_dodge(0.6), size = 3) +
...
Final_Estimate_Plot <- wrap_plots(list(Estimate_Plots_wrap, Plot_x_axis),
                         ...
                         heights = c(sum(heights), 1), 
                         guides  = "collect") +
  plot_annotation(title = "Firth-Penalized logistic regression results - Odds Ratio",
                  theme = theme(plot.title.position = "plot",
                                plot.title = element_text(size = 16, 
                                                          face = "bold", 
                                                          hjust = 0))) &
...
Estimate_Plots_log10 <- Estimate_Plots_wrap & 
  scale_x_log10()

Final_log10 <- wrap_plots(list(Estimate_Plots_log10, Plot_x_axis),
                         ncol = 1,
                         heights = c(sum(heights), 1), 
                         guides  = "collect") +
  plot_annotation(title = "Firth-Penalized logistic regression results - Odds Ratio",
