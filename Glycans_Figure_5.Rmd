...
# write_csv(Reg_Results_Basic, file = "~/SLE/Glycans/Output_Data/Figure_5_GLMs/GLM_normal.csv")
...
# write_csv(Reg_Results_Firth, file = "~/SLE/Glycans/Output_Data/Figure_5_GLMs/GLM_Firth.csv")

## Save separately as excel files ##
# Reg_Results_Firth_stats %>%
#   filter(Model == "T0") %>%
#   write_xlsx(x = .,
#            path = "~/SLE/Glycans/Output_Data/Figure_5_GLMs/Figure_5_GLM_Firth_Baseline.xlsx")
# 
# Reg_Results_Firth_stats %>%
#   filter(Model == "T12") %>%
#   write_xlsx(x = .,
#            path = "~/SLE/Glycans/Output_Data/Figure_5_GLMs/Figure_5_GLM_Firth_Month12.xlsx")
# 
# Reg_Results_Firth_stats %>%
#   filter(Model == "Delta") %>%
#   write_xlsx(x = .,
#            path = "~/SLE/Glycans/Output_Data/Figure_5_GLMs/Figure_5_GLM_Firth_Delta.xlsx")
...
Glycan_Annotations_clean <- read_xlsx("~/SLE/Glycans/Output_Data/Gl_annotations_clean.xlsx") %>% 
  ...
  unite(Marker, c(Origin, Marker), sep = "_", remove = TRUE)

# 1) Wrap your pipeline into a function
process_DF <- function(df) {
  df2 <- df %>%
    inner_join(Glycan_Annotations_clean, by = "Marker")
...
plot_GLM_results_final <- function(Dataframe_GLM, Method, Double_Sign_Markers) {
  ...
  Label_df <- if (Method == "T0") {
    DF_Plot %>% 
        filter(Significance == "Significant") %>% 
        mutate(Origin = gsub(pattern = "_.*", replacement = "", Marker), 
               Origin = str_trim(Origin), 
               Annotation = paste0(Origin, " ", str_trim(Glycan_Annotation)))
  } else {
    DF_Plot %>%
      filter(Highlight == "Significant in both<br>response types")%>% 
      mutate(Origin = gsub(pattern = "_.*", replacement = "", Marker), 
             Origin = str_trim(Origin), 
             Annotation = paste0(Origin, " ", str_trim(Glycan_Annotation)))
  }
...
Yeet <- wrap_plots(Double_T0, Double_M12, Double_delta, 
           ncol = 1, 
           guides = "collect") +
...
ggsave(filename = "~/SLE/Glycans/Plots_2/Figure_5_GLMs/Firth_BM12Delta_Glycans_A4.jpeg",
