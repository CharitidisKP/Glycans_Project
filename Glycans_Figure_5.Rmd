...
plot_GLM_results <- function(Dataframe_GLM, Method, Model_Type, Double_Sign_Markers) {
  ...
  Sign_Markers_current <- DF_Plot %>% 
    ...
    mutate(Model = sub("Log2FC", "Log<sub>2</sub>FC", Model)) %>% 
    mutate(Highlight = case_when(Marker %in% Sign_Markers_current ~ "Significant in both models", 
  ...
  Double_sign_Plot <- ggplot(DF_Plot_clean, aes(x = log10(OR), y = -log10(p.value))) +
    ...
    geom_text_repel(data = DF_Plot_clean %>% filter(Marker %in% Sign_Markers_current), 
                    ...
                    max.overlaps = Inf, 
                    min.segment.length = 0) +
    scale_colour_manual(values = c("Significant in both models" = "deeppink3", 
...
plot_GLM_results_final <- function(Dataframe_GLM, Method, Double_Sign_Markers) {
  ...
  lgnd_cols <- c("Significant in both" = "deeppink3",
                 "Significant in clinical" = "royalblue", 
                 "Significant in histological" = "mediumspringgreen",
  ...
  DF_Plot <- Dataframe_GLM %>% 
    ...
    mutate(Model = sub("Log2FC", "Log<sub>2</sub>FC", Model), 
             Highlight = case_when(clinical_sig & histo_sig  ~ "Significant in both",
                                   clinical_sig & !histo_sig ~ "Significant in clinical", 
                                   !clinical_sig & histo_sig ~ "Significant in histological",
                                   TRUE ~ "Not significant"), 
             Label = gsub("_", " ", Marker), 
             Highlight = factor(Highlight, levels = c("Significant in both", 
                                                      "Significant in clinical",
                                                      "Significant in histological",
                                                      "Not significant")))
  ...
  Double_sign_Plot <- ggplot(DF_Plot, aes(x = log10(OR), y = -log10(p.value))) +
    ...
    geom_text_repel(data = DF_Plot %>% filter(Highlight == "Significant in both"), 
                    aes(label = Label), 
                    color = "black", 
                    size = 4, 
                    force = 2,
