...
plot_GLM_results <- function(Dataframe_GLM, Model_Type, Double_Sign_Markers) {
DF_Plot <- Dataframe_GLM %>% 
    filter(Model != "T0") %>% 
    mutate(Model = sub("Log2FC", "Log<sub>2</sub>FC", Model)) %>% 
    mutate(Highlight = if_else(Marker %in% Double_Sign_Markers, 
                               "Present in both models", 
                               "Not present in both models"), 
           Label = gsub("_", " ", Marker))
  
    ggplot(DF_Plot, aes(x = log10(OR), y = -log10(p.value))) +
    geom_point(aes(color = Highlight, alpha = Significance), size = 2) +
    geom_text_repel(data = DF_Plot %>% filter(Marker %in% Double_Sign_Markers, 
    ...
    scale_colour_manual(values = c("Present in both models" = "coral1", 
                                   "Not present in both models" = "grey"), 
                        name = "Marker type") +
    scale_alpha_manual(values = c("Significant" = 1.0, 
                                  "Not Significant" = 0.2), 
                                  guide = "none") +
  ...
  labs(title = paste0("Logistic regression results - ", Model_Type," model
    ...
    geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "black") +  
    geom_vline(xintercept = 0, linetype = "dashed", color = "black") +  
    facet_wrap(Outcome ~ Model, ncol = 3, scales = "free_x") +
    theme_minimal(base_size = 14) +
    theme(plot.title = element_markdown(hjust = 0, face = "bold", size = 14),
          legend.title = element_markdown(face = "bold"), 
          legend.text = element_markdown(face = "bold"), 
...
Firth_GLM_Plot_Double_sign <- plot_GLM_results(Reg_Results_Firth_stats, "Firth Penalised", Double_Sign_Markers)

ggsave(filename = paste0("~/SLE/Glycans/Plots_2/Figure_5_GLMs/Volcano_Firth_GLM_Double_sign.jpeg"),
       plot = Firth_GLM_Plot_Double_sign, width = 22, height = 15, dpi = 300, device = "jpeg")
