...
NIH_Test_Results <- AI_CI_data_clean %>% 
  ...
  summarize(NIH_Test = list(wilcox.test(Value ~ Result_Value, 
                                 ...
                                 exact = FALSE)), 
            .groups = "drop") %>% 
  mutate(NIH_Test = map(NIH_Test, tidy)) %>% 
  unnest(NIH_Test, names_sep = "_") %>%
  group_by(Time_point) %>%
  mutate(NIH_FDR = p.adjust(NIH_Test_p.value, method = "fdr")) %>%
  ungroup()

NIH_Test_Results %>% 
  filter(NIH_Test_p.value < 0.05)
