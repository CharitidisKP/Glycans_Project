...
Glycan_Annotations_clean <- read_xlsx("~/SLE/Glycans/Output_Data/Gl_annotations_clean.xlsx") %>% 
  mutate(Glycan_Annotation = gsub(" - Multiple Structures", "", Glycan_Annotation)) %>% 
...
# write_csv(x = Cox_Results_Uni_Total, file = "~/SLE/Glycans/Output_Data/Figure_6_Cox/Cox_Univariable.csv")

# write_xlsx(x = Cox_Results_Uni_Total,
#            path = "~/SLE/Glycans/Output_Data/Figure_6_Cox/Figure_6_Cox_Univariable.xlsx")
...
# write_csv(x = Cox_Results_Multi_Total_strict, 
#           file = "~/SLE/Glycans/Output_Data/Figure_6_Cox/Cox_Multivariate_Sign_Markers.csv")
...
Plot_Cox_Results <- function(Plot, Type, Filter) {
 Plot %>% 
  ...
  mutate(Origin = factor(Origin, levels = c("AGP", "C3", "Fab", "Fc", "IgG", "Serum")), 
         ...
         Glycan_Annotation = if_else(p.value < 0.05, 
                                     paste0("<span style='font-size:18px'>* </span>", Glycan_Annotation),
                                            Glycan_Annotation)) %>% 
    { if (Filter) filter(., p.value < 0.05) else . } %>% 
  ggplot(., aes(x = hazard_ratio, y = Glycan_Annotation, colour = Comparison)) +
...
Cox_Uni_plot <- Plot_Cox_Results(Cox_Results_Uni_Total, "Univariable", Filter = TRUE)
Cox_Multi_plot <- Plot_Cox_Results(Cox_Results_Multi_Total_strict, "Multivariable", Filter = FALSE)
...
```{r Visualise normal Cox results - Manual}
 ...
 Cox_Results_Multi_Total_strict %>% 
  ...
  ggplot(., aes(x = hazard_ratio, y = Marker, colour = Comparison)) +
      ...
      facet_grid(Origin ~ ., scales = "free_y", space = "free_y") +
      labs(title = "Multivariable Cox regression: Hazard ratios with 95% CI", 
...
# write_csv(x = Cox_Results_Multi_firth,
#           file = "~/SLE/Glycans/Output_Data/Figure_6_Cox/Cox_Multivariate_Sign_Markers_Firth.csv")

# write_xlsx(x = Cox_Results_Multi_firth,
#            path = "~/SLE/Glycans/Output_Data/Figure_6_Cox/Figure_6_Cox_Multivariable_Sign_Markers_Firth.xlsx")
...
```{r Alternative Multivariate Cox visualisation prep}
...
Cox_Results_Multi_firth_clean_2 <- Cox_Results_Multi_firth_clean %>%
  filter(Comparison == "Delta") %>%
  separate(Marker, c("Origin", "Marker"), sep = "_", extra = "merge") %>% 
  inner_join(Glycan_Annotations_clean, by = c("Origin", "Marker")) %>% 
  mutate(Origin = factor(Origin, levels = c("AGP", "C3", "Fab", "Fc", "IgG", "Serum")), 
...
Cox_Multi_firth_plot2 <- ggplot(Cox_Results_Multi_firth_clean_2, 
                                aes(x = hazard_ratio, y = Glycan_Annotation, colour = Comparison)) +
  geom_point(position = position_dodge(0.6), size = 3, na.rm = TRUE) +
  geom_errorbarh(data = . %>% filter(!is.na(CI_lower) & !is.na(CI_upper)),
                 aes(xmin = CI_lower, xmax = CI_upper),
                 position = position_dodge(0.6),
  ...
  scale_y_discrete(drop = FALSE) +
  facet_grid(Origin ~ ., scales = "free_y", space = "free_y", drop = FALSE) +
  labs(title = "Multivariable Cox regression - Firth penalised: Hazard ratios with 95% CI", 
       caption = paste0("Only markers which were significant in the non-penalised univariate analysis shown
                        <br>T0 and T12 values did not converge and were ommited"),
       x = "Hazard ratio") +
  theme_minimal(base_size = 11, base_family = "sans") +
...
#        filename = "~/SLE/Glycans/Plots_2/Figure_6_Survival/Combined_firth_Glycans_A4.jpeg",
...
Run_multi_coxf2 <- function(Top_Markers) {

  by_comp <- Joint_DF %>%
    select(ID, Time, Event, Marker, T0, T12, Delta) %>%
    pivot_longer(cols = -c(ID, Time, Event, Marker),
  ...
  # tidy results (same as your current function)
  results <- by_comp %>% 
    mutate(Tidy = map(Fit, ~ {
      fit <- .x
      est <- coef(fit)                         # log-HR
      ...
      se <- sqrt(diag(vcov(fit)))
      stat <- est / se
      lower <- fit$ci.lower                    # already on HR scale
      upper <- fit$ci.upper                    # already on HR scale
      pval  <- fit$prob
      
      tibble(Marker = term,
             hazard_ratio_log = est,
             std.error = se,
             statistic = stat,
             p.value = pval,
             CI_lower = lower,
  ...
  # return both
  list(results = results, fits = by_comp %>% select(Comparison, Fit))
}

Cox_multi_firth <- Run_multi_coxf2(Top_Markers_strict)
Cox_Results_Multi_firth <- Cox_multi_firth$results
Cox_Fits_Multi_firth <- Cox_multi_firth$fits
...
## use your existing fit
fit_delta <- Cox_Fits_Multi_firth %>% dplyr::filter(Comparison=="Delta") %>% dplyr::pull(Fit) %>% .[[1]]
fit_delta <- attach_inputs(fit_delta, "Delta")  # from your helpers; just to compute N below

# wrap tidy_fun so extra args are harmless
tidy_wrap <- function(x, ...) tidy_coxphf(x, ...)

tbl_delta <- gtsummary::tbl_regression(
  fit_delta,
  exponentiate = TRUE,
  tidy_fun = tidy_wrap,   # <— use the wrapper
  estimate_fun = ~gtsummary::style_ratio(.x, digits = 2),
  pvalue_fun = ~gtsummary::style_pvalue(.x, digits = 2)) %>%
  gtsummary::add_q(method = "fdr") %>%
  gtsummary::modify_fmt_fun(q.value ~ gtsummary::style_pvalue(digits = 2)) %>%
  gtsummary::modify_header(gtsummary::all_stat_cols() ~ "**HR (95% CI)**") %>%
  gtsummary::add_n(location = "header") %>%
  gtsummary::modify_caption("**Multivariable Cox with Firth (Delta)**")
```
