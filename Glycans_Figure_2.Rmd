...
Correlation_Results_Clean <- read_csv(file = "~/SLE/Glycans/Output_Data/Figure_2_Clinical_Cors/Figure_2_Glycan_Clinical_Correlations.csv")
...
Main_plot <- ggplot(Correlation_Results_Clean, aes(x = Comparator, y = Marker, fill = Correlation)) +
  ...
  geom_text(aes(label = Significance, vjust = 0, hjust = 0.5, fontface = "bold", size = 4.5)) +
...
DF <- Correlation_Results_Clean_Sign %>%
...
Main_plot <- ggplot(DF, aes(x = Comparator, y = RowKey, fill = Correlation)) +
  geom_tile(color = "white", width = 1, height = 1) +
  scale_fill_gradient2(low = "dodgerblue", mid = "white", high = "firebrick", 
                       midpoint = 0, breaks = seq(-1, 1, by = 0.5), 
                       guide = guide_colorbar(order = 1, title.position = "top", title.hjust = 0, 
  ...
  ## For some god forsaken reason they are aligned properly on the y axis with ##
  ## vjust = 0.8 when using scale = 2. If scale = 1 try either 0.8 or 0.5 ##
  # scale_y_discrete(labels = function(x) gsub("^[^_]*_", "", x), expand = c(0,0)) +
...
# write_xlsx(x = Correlation_Results_Clean_Sign,
#            path = "~/SLE/Glycans/Output_Data/Figure_2_Clinical_Cors/Figure_2_Glycan_Clinical_Correlations.xlsx")

# Correlation_Results_Clean_Sign %>% 
#   filter(Comparison == "Baseline vs Baseline") %>% 
#   write_xlsx(x = ., 
#              path = "~/SLE/Glycans/Output_Data/Figure_2_Clinical_Cors/Figure_2_Glycan_Clinical_Correlations_BvsB.xlsx")

# Correlation_Results_Clean_Sign %>% 
#   filter(Comparison == "Month 12 vs Month 12") %>% 
#   write_xlsx(x = ., 
#              path = "~/SLE/Glycans/Output_Data/Figure_2_Clinical_Cors/Figure_2_Glycan_Clinical_Correlations_M12vsM12.xlsx")

# Correlation_Results_Clean_Sign %>% 
#   filter(Comparison == "Baseline vs Month 12") %>% 
#   write_xlsx(x = ., 
#              path = "~/SLE/Glycans/Output_Data/Figure_2_Clinical_Cors/Figure_2_Glycan_Clinical_Correlations_BvsM12.xlsx")

# ggsave(filename = "~/SLE/Glycans/Plots_2/Figure_2_Correlations_Total/Correlations_significant_Glycans_A4.jpeg",
...
```{r Sub correlation plots, one by one}
