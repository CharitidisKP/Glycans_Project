...
# 1) Wrap your pipeline into a function
process_DF <- function(df) {
  ...
  row_levels <- DF %>%
    arrange(Origin, across(any_of(c("Comparator", "Histology"))), Marker) %>%
    pull(RowKey) %>%
    unique()
  ...
  DF <- DF %>%
    mutate(RowKey = factor(RowKey, levels = row_levels), 
           ...
           Glycan_Annotation = str_pad(Glycan_Annotation, width = max_len, side = "left", pad = "\u00A0"),
           RowKey = str_pad(as.character(RowKey), width = max_len, side = "left", pad = "\u00A0"))
  
  return(list(DF = DF, lab_map = lab_map))
}

Correlations_Delta_Sign_2_processed <- process_DF(Correlations_Delta_Sign_2)
Glycan_Deltas_vs_M12_Cor_Sign_2_processed <- process_DF(Glycan_Deltas_vs_M12_Cor_Sign_2)
Delta_Hist_Cor_Res_Sign_2_processed <- process_DF(Delta_Hist_Cor_Res_Sign_2)
...
Correlations_Delta_Sign_plot <- ggplot(Correlations_Delta_Sign_2_processed$DF, 
                                       aes(x = Comparator, y = RowKey, fill = Correlation)) +
  ...
  geom_text(aes(label = Significance), vjust = 0.8, hjust = 0.5, fontface = "bold", size = 7) +
  # scale_y_discrete(labels = function(x) gsub("^[^_]*_", "", x), expand = c(0,0)) +
  scale_y_discrete(labels = Correlations_Delta_Sign_2_processed$lab_map, expand = c(0,0)) +
...
Glycan_Deltas_vs_M12_Cor_Sign_plot <- ggplot(Glycan_Deltas_vs_M12_Cor_Sign_2_processed$DF, 
                                             aes(x = Comparator, y = RowKey, fill = Correlation)) +
  geom_tile(color = "white", width = 1, height = 1) +
  scale_fill_gradient2(low = "dodgerblue", mid = "white", high = "firebrick", 
                       limits = c(Corr_scale$min_corr, Corr_scale$max_corr),
  ...
  # scale_y_discrete(labels = function(x) gsub("^[^_]*_", "", x), expand = c(0,0)) +
  scale_y_discrete(labels = Glycan_Deltas_vs_M12_Cor_Sign_2_processed$lab_map, expand = c(0,0)) +
...
Plot_delta_hist_cor <- function(Dataframe_clean, lab_map, Time, Title) {
  ...
  this_plot <- ggplot(Dataframe_clean_2, aes(x = Histology, y = RowKey, fill = Correlation)) +
    geom_tile(color = "white", width = 1, height = 1) +
    scale_fill_gradient2(low = "dodgerblue", mid = "white", high = "firebrick", 
                         limits = c(Corr_scale$min_corr, Corr_scale$max_corr),
                         midpoint = 0, breaks = seq(-1, 1, by = 0.5), 
    ...
    scale_y_discrete(labels = lab_map, expand = expansion(add = 0.8)) +
    ...
    theme_minimal(base_size = 11, base_family = "sans") +
    theme(strip.placement = "outside",
          strip.text.y.right = element_markdown(size = 11, 
                                                angle = 0, 
                                                hjust = 0.5, 
                                                vjust = 0.5, 
                                                face = "bold", 
...
Delta_Hist_T0_cor_plot <- Plot_delta_hist_cor(Delta_Hist_Cor_Res_Sign_2_processed$DF, 
                                              Delta_Hist_Cor_Res_Sign_2_processed$lab_map, 0, "baseline")
Delta_Hist_T12_cor_plot <- Plot_delta_hist_cor(Delta_Hist_Cor_Res_Sign_2_processed$DF, 
                                               Delta_Hist_Cor_Res_Sign_2_processed$lab_map, 12, "month 12")

```

```{r Combined F4 A4}

p1 <- Correlations_Delta_Sign_plot + labs(tag = "A")
p2 <- Glycan_Deltas_vs_M12_Cor_Sign_plot + labs(tag = "B")
p3 <- Delta_Hist_T0_cor_plot + labs(tag = "C")
p4 <- Delta_Hist_T12_cor_plot + labs(tag = "D")

Gabagool <- wrap_plots(p1 + p2,
                       p3, p4, 
                       ncol = 1, 
                       heights = c(0.6, 1.3, 0.75),
                       guides = "collect") +
...
#        filename = "~/SLE/Glycans/Plots_2/Figure_4_Delta_Correlations/Delta_Corr_Combined_Glycans_A4.jpeg",
#        width = 210, height = 297, units = "mm", scale = 2, dpi = 300, device = "jpeg")

```

```{r Save the excels}
# Correlations_Delta_Sign_2, Glycan_Deltas_vs_M12_Cor_Sign_2, Delta_Hist_Cor_Res_Sign_2

# write_xlsx(x = Correlations_Delta_Sign_2,
#            path = "~/SLE/Glycans/Output_Data/Figure_4_Delta_Cors/Figure_4_Glycan_Delta_Clinical_Delta_Correlations.xlsx")
# 
# write_xlsx(x = Glycan_Deltas_vs_M12_Cor_Sign_2,
#            path = "~/SLE/Glycans/Output_Data/Figure_4_Delta_Cors/Figure_4_Glycan_Delta_Clinical_M12_Correlations.xlsx")
# 
# Delta_Hist_Cor_Res_Sign_2 %>% 
#   filter(Time_label == "0") %>% 
#   write_xlsx(x = .,
#            path = "~/SLE/Glycans/Output_Data/Figure_4_Delta_Cors/Figure_4_Glycan_Delta_NIH_Baseline_Correlations.xlsx")
# 
# Delta_Hist_Cor_Res_Sign_2 %>% 
#   filter(Time_label == "12") %>% 
#   write_xlsx(x = .,
#            path = "~/SLE/Glycans/Output_Data/Figure_4_Delta_Cors/Figure_4_Glycan_Delta_NIH_Month12_Correlations.xlsx")
