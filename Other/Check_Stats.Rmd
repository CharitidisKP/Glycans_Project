...
Fig_2_imp <- Fig_2_data %>% 
  ...
  select(Marker, Comparator, Comparison, Correlation, p.value, Significance) %>% 
...
Fig_3_Group1 <- Fig_3_data %>% 
  ...
  select(Marker, Result, Comparison, Correlation, p.value, Significance) %>% 
...
Fig_4A_imp <- Fig_4A_data %>% 
  select(Origin, Marker, Comparator, Correlation, p.value, Significance) %>% 
...
Fig_4C_imp <- Fig_4C_data %>% 
  select(Origin, Marker, Histology, Correlation, p.value, Significance) %>% 
...
Fig_5B_data <- Fig_5_data %>% 
  mutate(Significance = case_when(p.value < 0.05 ~ "*")) %>% 
  select(Marker, Outcome, Model, estimate, statistic, p.value, Significance) %>% 
  filter(Significance == "*", 
         Model == "T12")
...
Fig_6A_imp <- Fig_6A_data %>% 
  mutate(CI_lower = conf.low, 
         CI_upper = conf.high) %>% 
  select(Marker, Comparison, p.value, hazard_ratio, CI_lower, CI_upper) %>% 
  filter(p.value < 0.05 
         # ,Comparison == "Delta"
         ) %>% 
  separate(Marker, into = c("Origin", "Marker"),
...
Glycosylation_Data_Corrected <- read_xlsx("~/SLE/Glycans/Glycosylation_Data_Corrected.xlsx")
Markers_of_Interest_annotated <- read_xlsx("~/SLE/Glycans/Markers_of_Interest_annotated.xlsx")

Sheets_imp <- list_modify(Sheets_imp, 
                          Markers_of_Interest = Markers_of_Interest, 
                          Annotations = Markers_of_Interest_annotated, 
                          Glycosylation_Data_Corrected = Glycosylation_Data_Corrected)

# write_xlsx(Sheets_imp, path = "~/SLE/Glycans/Markers_of_Interest_with_stats.xlsx")
...
df <- read_xlsx(path = "~/SLE/Glycans/Output_Data/Figure_6_Cox/Figure_6_Cox_Univariable.xlsx") %>%
  separate(col = Marker, into = c("Origin", "Marker"), sep = "_") %>% 
  filter(p.value < 0.05)

df %>%
  select(Origin, Marker, p.value, hazard_ratio, conf.low, conf.high) %>% 
filter(Marker == "IORMIF1N5H6S2F1")
